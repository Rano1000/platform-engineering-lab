...
After a recovery has replaced Pods, use `make kindnet-policy-validate` only under a separate runtime approval. The validation target discovers the new UIDs and does not require the deleted identities. A `diagnostic path escapes its approved run directory` failure indicates an artifact-root mismatch or unsafe path, not a NetworkPolicy result. Preserve that incomplete directory. The harness now passes the kindnet run directory as the explicit artifact boundary while retaining traversal, symlink, special-file, missing-directory, and prefix-collision rejection.

If DNS succeeds but evidence capture reports a missing diagnostic suffix, preserve the Pod and evidence. Canonical names are now returned by the shared diagnostic helper for both capture and sanitization; accepting legacy alternatives or manually renaming incomplete evidence would bypass completeness validation.
...
Docker can restart retained kind containers with different bridge addresses. A stale policy checksum or `/32` makes `make gitops-validate` fail and may prevent the controller or server from reaching the translated API endpoint. Inspect all platform health first: endpoint drift can coincide with wider kindnet and DNS disruption, which policy reconciliation does not repair. With separate approval, `make gitops-api-policy-reconcile` performs a three-snapshot, UID- and resource-version-guarded update of only the three Argo API policies. Never use the full installer, widen the CIDR, or manually patch a mixed policy set. Review `.artifacts/gitops-api-policy-reconcile/` if the transaction stops.

Argo validation reads workload generation, replica counters, selected Pod phases, container readiness and waiting reasons, and restart growth. A rollout status that appears complete while a repository-server Pod is in `CrashLoopBackOff` is a failure, not a healthy controller.
