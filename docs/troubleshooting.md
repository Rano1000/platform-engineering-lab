...
If GitOps validation reports that `AppProject/default` is permissive, missing, or owned unexpectedly, do not bootstrap an Application. Compare it with `platform/addons/argocd/default-project.yaml`. For an already installed reviewed controller, `make gitops-default-project-harden` performs an exact-context, exact-confirmation, server-side apply and verifies the deterministic checksum. It refuses unrelated ownership and never deletes the built-in project.
