...
Guarded diffs set Argo CD's supported `--diff-exit-code` to `20`, keeping expected differences distinct from operational errors. Exit `20` is successful only after semantic validation. Before bootstrap, the root diff may contain one complete creation of `Application/gitops/golden-path-api` matching the approved immutable manifest. After bootstrap, a checksummed live-state snapshot must prove that the child exists; the root diff may then contain only the explicitly approved chart-revision annotation and target-revision transition. Creation against an existing child, modification against an absent child, deletion, an unknown resource, empty output, malformed output, or any operational error fails closed. The child diff may contain only creations or modifications from its exact rendered resource set. Sanitized before/after objects, lifecycle decisions, field differences, stderr, and original status are retained for diagnosis.

Each attempt atomically retains sanitized raw output, command status, canonical approved and proposed Applications, and a checksummed JSON-Pointer difference report beneath `.artifacts/gitops-diff`. Differences are classified as defaulted fields, missing-versus-empty representation, Argo-added metadata, or genuine desired-state changes, but classification never makes a difference acceptable. The protected child-specification checksum remains mandatory.

Root diff normalization permits exactly one runtime field: Argo's `argocd.argoproj.io/tracking-id` annotation with value `platform-environment:argoproj.io/Application:gitops/golden-path-api`. The value is derived from the protected root, API group, kind, namespace, and child name. It must be absent from the repository manifest and present exactly once in Argo's proposal. Every altered delimiter or identity component and every other additional metadata field remains forbidden. The decision is retained in `normalization-decisions.json` and does not alter the protected specification checksum.
