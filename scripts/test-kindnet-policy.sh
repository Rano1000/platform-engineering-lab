...
for knp_index in 0 1; do
  ...
  kubectl_lab run "$knp_name" -n gitops --restart=Never --image="$knp_image" \
    ...
    --overrides="$knp_override" -o json >"$knp_temporary/$knp_name-created.json"
  knp_uid=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["metadata"]["uid"])' "$knp_temporary/$knp_name-created.json")
  ...
  kubectl_lab get pod "$knp_name" -n gitops -o json >"$knp_temporary/$knp_name-pod.json"
  kubectl_lab describe pod "$knp_name" -n gitops >"$knp_temporary/$knp_name.describe" 2>&1
  kubectl_lab get events -n gitops --field-selector "involvedObject.uid=$knp_uid" -o json >"$knp_temporary/$knp_name-events.json"
  for knp_dns_file in created.json log pod.json describe events.json; do
    knp_dns_source="$knp_temporary/$knp_name-${knp_dns_file}"
    [ "$knp_dns_file" = log ] && knp_dns_source="$knp_temporary/$knp_name.log"
    python3 "$SCRIPT_DIR/redact-network-diagnostics.py" --sanitize "$knp_dns_source" "$knp_root/$knp_name-$knp_dns_file"
  done
  python3 "$SCRIPT_DIR/kindnet-dns-probe.py" validate --log "$knp_root/$knp_name-log" \
    --pod "$knp_root/$knp_name-pod.json" --node "$knp_node"
