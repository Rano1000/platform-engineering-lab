...
kubectl_lab get pods -n "$KPR_NAMESPACE" -l app=kindnet -o json >"$kpr_raw/pods.json"
kubectl_lab logs -n "$KPR_NAMESPACE" -l app=kindnet --all-containers --since=15m >"$kpr_raw/kindnet.log" 2>&1 || true
...
python3 "$SCRIPT_DIR/validate-kindnet-recovery.py" plan --identity "$kpr_raw/identity.json" | while IFS='|' read -r kpr_node kpr_name kpr_uid; do
  ...
  kubectl_lab wait -n "$KPR_NAMESPACE" --for=condition=Ready pod -l app=kindnet \
    --field-selector "spec.nodeName=$kpr_node" --timeout="${KPR_TIMEOUT_SECONDS}s"
  kubectl_lab get pods -n "$KPR_NAMESPACE" -l app=kindnet --field-selector "spec.nodeName=$kpr_node" -o json >"$kpr_raw/replacement.json"
  python3 "$SCRIPT_DIR/validate-kindnet-recovery.py" replacement --pods "$kpr_raw/replacement.json" \
    ...
    --daemonset-uid "$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["daemonSet"]["uid"])' "$kpr_raw/identity.json")"
done

kubectl_lab get daemonset "$KPR_DAEMONSET" -n "$KPR_NAMESPACE" -o json >"$kpr_raw/daemonset-after.json"
kubectl_lab get pods -n "$KPR_NAMESPACE" -l app=kindnet -o json >"$kpr_raw/pods-after.json"
kubectl_lab logs -n "$KPR_NAMESPACE" -l app=kindnet --all-containers --since=15m >"$kpr_raw/kindnet-after.log" 2>&1 || true
